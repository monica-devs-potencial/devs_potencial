...
(function () {
  document.addEventListener('DOMContentLoaded', () => {
    const overlay = document.getElementById('sobreModal');
    const btnOpen = document.getElementById('btnSobre');
    const btnClose = document.getElementById('btnSobreClose');
    if (!overlay) return;
    ...
    if (btnOpen)  btnOpen.addEventListener('click', openSobre);
    if (btnClose) btnClose.addEventListener('click', closeSobre);

    overlay.addEventListener('click', (e) => {
      if (e.target === overlay) closeSobre();
    });

    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape' && overlay.classList.contains('open')) closeSobre();
    });
  });
})();
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function beep() {
    try {
      ...
      // em alguns browsers o contexto inicia "suspended"
      ...
      g.gain.value = 0.03;     // volume (bem baixinho)
  ...
  document.addEventListener('click', function (e) {
    ...
    // opcional: ignore links do admin discreto
    // if (el.getAttribute('href') === 'admin/login.php') return;
